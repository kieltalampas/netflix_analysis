...
SELECT *
FROM durations
WHERE duration_minutes = (SELECT MAX(duration_minutes) FROM durations);
...
WITH season_cte AS (
	SELECT 
	    title,
	    CAST(REPLACE(REPLACE(duration, ' Seasons', ''), ' Season', '') AS INT) AS duration_seasons
	FROM netflix
	WHERE type = 'TV Show'
)
...
-- 13. Find how many movies actor 'Chris Hemsworth' appeared in last 10 years!

SELECT *
FROM netflix
WHERE casts ILIKE '%Chris Hemsworth%'
	AND
